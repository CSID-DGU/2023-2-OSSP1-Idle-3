...
def getStepNodeJson():
  data = []
  with open("step_raw_data.csv", 'r') as csv_file:
  ...
  for i in range(len(node_reader)):
    sigoongu = node_reader[i]["SiGoonGu Name"]
    if sigoongu == "중구":
  ...
  # 노드 json 쓰기
  with open("result_data/step_node_in_Joongu.json", 'w', encoding = 'utf-8') as json_file:
