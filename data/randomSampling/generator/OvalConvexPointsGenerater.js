 ...
 */
module.exports = class OvalConvexPointsGenerater {
    /**
     * n각형을 이루는 
    ...
    generate(numVertices, minLat, maxLat, minLng, maxLng) {
        ...
        let points = angles.map(angle => {
            let lat = parseFloat((Math.cos(angle) * latRange / 2 + (minLat + maxLat) / 2).toFixed(10));
            let lng = parseFloat((Math.sin(angle) * lngRange / 2 + (minLng + maxLng) / 2).toFixed(10));    
            return { lat, lng };
        });
        return points;
    }
